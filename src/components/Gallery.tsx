...
export default function Gallery() {
  ...
  // Auto-play interval of 7 seconds (7000ms)
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex((prev) => (prev === GALLERY_IMAGES.length - 1 ? 0 : prev + 1));
    }, 7000);
    return () => clearInterval(timer);
  }, [currentIndex]); // Resets the 7s countdown whenever manual action changes currentIndex
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="card-neo-flat p-4 sm:p-6 md:p-8">
            ...
            {/* Viewport with dynamic fluid aspect ratio and containing original image formats to prevent cropping */}
            <div className="relative aspect-[4/3] sm:aspect-[16/10] md:aspect-[16/9] w-full overflow-hidden rounded-2xl bg-brand-bg/40 border border-white/20 group">
              {GALLERY_IMAGES.map((img, index) => (
                ...
                >
                  ...
                  {/* Micro-Interaction Overlay shown on hover on desktop */}
                  <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none flex items-center justify-center">
                    <span className="px-4 py-2 rounded-xl bg-white/95 text-brand-dark text-xs font-semibold shadow-md flex items-center gap-1.5 backdrop-blur-sm">
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 p-3 rounded-xl bg-black/40 text-white/90 hover:bg-black/60 backdrop-blur-sm transition-colors focus:ring-2 focus:ring-brand-gold/50 focus:outline-none"
                  >
                    <Maximize2 size={16} />
                  </button>
                </div>
              ...
              {/* Navigation Arrows (designed for touch friendly targets) */}
              <button
                onClick={prevSlide}
                aria-label="Foto anterior do consultório"
                className="absolute left-3 top-1/2 -translate-y-1/2 w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-white/80 hover:bg-white text-brand-dark shadow-md hover:shadow-lg transition-all flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-brand-gold z-10"
              >
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-white/80 hover:bg-white text-brand-dark shadow-md hover:shadow-lg transition-all flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-brand-gold z-10"
              >
                  ...
                  {GALLERY_IMAGES.map((img, idx) => (
                    <button
                      ...
                      className={`relative w-16 h-10 sm:w-20 sm:h-12 rounded-lg overflow-hidden flex-shrink-0 transition-all focus:outline-none ${
                        idx === currentIndex
                          ? "ring-2 ring-brand-gold scale-105 border-transparent shadow-md"
                          : "opacity-65 hover:opacity-90 border border-brand-dark/10"
                      }`}
                    >
                      <img
                        src={img.url}
